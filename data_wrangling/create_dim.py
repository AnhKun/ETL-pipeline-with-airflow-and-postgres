...
def lowercase_columns():
    df = pd.read_csv('../data/jantojun2020.csv')
    df.columns = df.columns.map(lambda x: x.lower())
    df.to_csv('../data/jantojun2020.csv', index=False)
...
def create_port_loc_df(path_csv):
    ...
    df.to_csv('../data/port_loc.csv', index=False)
...
def create_airline_df(path_txt):
        with open(path_txt) as f:
            ...
            airline_df.to_csv("../data/airline.csv", index=False)
...
def create_cancellation_ds(path_txt):
        ...
        with open(path_txt) as f:
            ...
            cancel_df.to_csv("../data/cancellation.csv", index=False)
